        ...
        if self.tipo == TIPO.DECIMAL:
            if self.expresion.tipo == TIPO.ENTERO:
                try:
                    ...
                    return Excepcion("Semantico", "No se puede castear para Floatq.", self.fila, self.columna)
        ...
        if self.tipo == TIPO.ENTERO:
            if self.expresion.tipo == TIPO.DECIMAL:
                try:
                    ...
                    return Excepcion("Semantico", "No se puede castear para Intq.", self.fila, self.columna)
            elif self.expresion.tipo == TIPO.CADENA:
                try:
                    return int(self.obtenerVal(self.expresion.tipo, val))
                except:
                    return Excepcion("Semantico", "No se puede castear para Intq.", self.fila, self.columna)
            return Excepcion("Semantico", "Tipo Erroneo de casteo para Intq.", self.fila, self.columna)
